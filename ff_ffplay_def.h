...
{
    double pts;   //时钟基础，当前帧显示时间戳，播放后，当前帧变成上一帧
...
{
    MyAVPacketList *first_pkt,*last_pkt; //队首尾指针
    int nb_packets;     //包数量，也就是队列元素数量
    int size;           //队列所有元素的数据大小总和
    int64_t duration;   //队列所有元素播放时间
    int abort_request;  //用户退出请求标志
    int serial;         //播放序列号，和MyAVPacketList的serial作用相同，但改变时序稍微有点不同
