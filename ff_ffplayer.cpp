...
int FFPlayer::stream_open(char *file_name)
{
    //初始化 audio video
...
void FFPlayer::stream_close()
{
    ...
    //close each stream
    if(audio_stream>=0)
        stream_component_close(audio_stream);//解码线程请求abort的时候有调用packet_queue_abort
    if(video_stream>=0)
    ...
    //释放frame队列
    frame_queue_destory(&pictq); //void frame_queue_destory(FrameQueue *f);
...
int FFPlayer::stream_component_open(int stream_index)
{
    ...
    //将码流中的编码器信息拷贝到新分配的编码器上下文结构体
    //比如codec_ctx->codec_id = codec_id;codec_ctx->codec_type = AVMEDIA_TYPE_AUDIO;codec_ctx->bit_rate = 128*1024;
    ret = avcodec_parameters_to_context(ctx,ic->streams[stream_index]->codecpar);
    if(ret<0)
        goto fail;
    //设置pkt_timebase
    ctx->pkt_timebase = ic->streams[stream_index]->time_base;
    ...
    switch(ctx->codec_type)
    {
        case AVMEDIA_TYPE_AUDIO:
            //从ctx(AVcodecContex)中获取音频格式参数
            sample_rate = ctx->sample_rate;
            nb_channels = ctx->channels;
            ...
            //初始化ffplay封装的音频解码器，并将解码器上下文ctx和decoder绑定
            auddec.decoder_init(ctx,&audioq);
            //启动音频解码线程 this代表的是当前对象FFPlayer
            auddec.decoder_start(AVMEDIA_TYPE_AUDIO,"audio_thread",this);
            // 允许音频输出
            //play audio 典型用法是在打开音频设备后
            //，可以先调用 SDL_PauseAudio(1) 来暂停音频回调，
            //然后进行初始化或加载音频数据等操作，然后再调用 SDL_PauseAudio(0)
            //来恢复音频回调，开始实际播放声音。
            SDL_PauseAudio(0);
            break;
        case AVMEDIA_TYPE_VIDEO:
            ...
            viddec.decoder_init(ctx,&videoq);
            // 启动视频频解码线程  this代表的是当前对象FFPlayer
...
//    } while (af->serial != is->audioq.serial);
                ...
                )
        {
            ...
            is->swr_ctx = swr_alloc_set_opts(NULL,
                                             ...
                                             0, NULL);
            //swr_alloc_set_opts正常的话,is->swr_ctx不为空，为空则报错
            if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {//重置把输入改了，输出没改 Initialize context after user parameters have been set.
        ...
        if (is->swr_ctx)
        {
            ...
            //out 输出缓冲区的指针数组，其中每个指针对应一个音频通道。只需设置第一个指针，用于存储转换后的音频数据。
        ...
        {
            // 未经重采样，则将指针指向frame中的音频数据 后面拷贝is->audio_buf为起始地址，长度为len1到SDL stream中
            is->audio_buf = af->frame->data[0]; // s16交错模式data[0], fltp data[0] data[1]
            resampled_data_size = data_size;
        }
        if(!isnan(af->pts))
           is->audio_clock = af->pts; //0?
        else
            is->audio_clock = NAN;
...
//    std::cout<<__FUNCTION__<<"leave"<<std::endl;
    ...
    //1创建上下文结构体，这个结构体是最上层的结构体，表示输入上下文
...
void FFPlayer::video_refresh(double *remaining_time)
{
    ...
    if(video_st)
    {
        ...
        std::cout << __FUNCTION__ << "vp->pts:" << vp->pts << " - af->pts:" << get_clock(&audclk) << ", diff:" << diff <<"audio pts"<<audclk.pts<< std::endl;
//        std::cout<<"test"<<"vpts"<<vp->pts<<" af->pts"
        if(diff > 0) { //视频超前audio就休眠
...
void FFPlayer::video_refresh_thread()
{
...
int FFPlayer::get_master_sync_type()
{
    if(av_sync_type == AV_SYNC_VIDEO_MASTER)
    {
        if(video_st)
            ...
            return AV_SYNC_AUDIO_MASTER; //如果没有视频成分则使用audio master
...
{
    //启动包队列 queue_是音/视频成员变量
    packet_queue_start(queue_);
    //创建线程 判断不同对象的codec_type来给各自的对象创建线程
    //this代表的是音频或者视频对象 arg(也是传递参数)代表的是FFPlayer对象
...
int FFPlayer::audio_open(int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate,AudioParams *audio_hw_params)
{
    ...
    //23.2ms ->46.4 每次读取的采样数量，多久产生一次回调和samples stream 是音频缓存区的数据指针
    ...
    //wanted_spec是期望参数，spec是实际参数，wanted_spec和spec都是SDL中的结构
...
int Decoder::decoder_decode_frame(AVFrame *frame)
{
    ...
    for(;;)
    {
        ...
        do
        {
            ...
            if(queue_->abort_request) //decoder_abort调用的时候 触发queue_abort_request为1
                return -1;
            ...
            case AVMEDIA_TYPE_AUDIO:
                ...
                if(ret>=0)
                {
                    ...
                    if(frame->pts != AV_NOPTS_VALUE)
                    {
                        ...
                        //int64_t a, AVRational bq, AVRational cq    `a * bq / cq`
...
//            tb = (AVRational){1,frame->sample_rate}; //设置为sample_rate为timebase
            ...
            if(!(af=frame_queue_peek_writable(&is->sampq))) //获取可写帧
                goto the_end;
            //3设置frame并放入FrameQueue
//            af->pts = (frame->pts == AV_NOPTS_VALUE)?NAN:frame->pts*av_q2d(tb); //转换时间戳
            af->pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);  // 转换时间戳
            AVRational tt;
            tt.den = frame->sample_rate;
            tt.num = frame->nb_samples;
//            af->duration = av_q2d((AVRational){frame->nb_samples,frame->sample_rate});
            af->duration = av_q2d(tt);
...
int Decoder::video_thread(void *arg)
{
    ...
    for(;;)  //循环取出视频解码的帧数据
    {
        ...
        if(!ret)           //没有得到解码画面，什么情况下会得不到解码后的帧
        {
            continue;
        }
     //1/25 = 0.04秒
     ...
     // 1/帧率 = duration 单位秒, 没有帧率时则设置为0, 有帧率帧计算出帧间隔
        AVRational tes;
        tes.num  = frame_rate.num;
        tes.den = frame_rate.den;
//        duration = (frame_rate.num && frame_rate.den ? av_q2d((AVRational){frame_rate.den,frame_rate.num}):0);
        duration = (frame_rate.num && frame_rate.den ? av_q2d(tes):0);
